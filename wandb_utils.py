...
def collect_wandb_runs(wandb_root: str | Path = "wandb") -> pd.DataFrame:
    ...
    for run_dir in sorted(p for p in base_path.iterdir() if p.is_dir() and p.name.startswith("run-")):
        ...
        # Only collect runs that progressed past step 198 (new epoch/step setting)
        # Some runs may not include an _step key or it may not be an int; handle safely.
        ...
        if step <= 198:
            # Skip runs that did not reach the new threshold
            continue
        ...
        imb_factor = args_map.get("imb_factor")
        new_data_aug = args_map.get("new_data_aug") or _extract_config_value(files_dir / "config.yaml", "new_data_aug")
